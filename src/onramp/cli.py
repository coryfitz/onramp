...
def run_ios():
    ...
    env = ensure_node_env()  # returns a dict you can pass to subprocess
    try:
    ...
    # 2) Xcode checks (no special env needed)
    try:
    ...
    # 3) Ensure native projects exist, using the same Node env
    if not ensure_native_projects(custom_env=env):
        print("Failed to set up iOS project.")
        return

    # 4) Simulators (no Node env required)
    ...
    # 5) Launch via RN CLI using the Node env
    try:
        ...
        if "DVTDownloads.framework" in error_output or "IDESimulatorFoundation" in error_output:
            ...
            # Use the same env for these commands as well if you call them
            print("1. Ensure Node is selected with your tool (ensure_node_env)")
...
def run_android():
    ...
    if not ensure_native_projects(env):
