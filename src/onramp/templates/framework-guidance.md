...
This file contains OnRamp's generated framework defaults. Do not customize it:
future upgrades manage it. Keep project-specific instructions in the root
`AGENTS.md`, preserve those constraints, and flag contradictory instructions
instead of silently discarding either set.
...
- `onramp ios --production` builds and runs an iOS Release app on a simulator
  ...
  archive, sign for distribution, or upload to TestFlight. Keep the command
  open for Xcode Device Hub clipboard sharing; Ctrl+C stops sharing but leaves
  the app installed.
