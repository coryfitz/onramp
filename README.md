...
First-time installs, architecture repairs, display-only device replacements,
and removal of old runtimes, images, or devices still require confirmation.
...
installed. Their removal still asks separately: an older device can contain
valuable app data, and a shared runtime may be needed by another project.
`--force` approves updates, **not cleanup**. Never equate an old AVD with an
unused image: multiple devices can share one installed system image.
...
`onramp~=0.5.30`. Patch releases remain compatible with that project schema;
